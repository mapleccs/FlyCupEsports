...
class Player(Base):
    ...
    SecondaryPositionId: Mapped[Optional[int]] = mapped_column(Integer, comment='副位置')
    CurrentRankId: Mapped[Optional[int]] = mapped_column(Integer, comment='当前段位')
    HighestRankId: Mapped[Optional[int]] = mapped_column(Integer, comment='最高段位')
    QQ: Mapped[str] = mapped_column(VARCHAR(20), comment='QQ号')
    Contact: Mapped[Optional[str]] = mapped_column(VARCHAR(20), comment='联系方式')
